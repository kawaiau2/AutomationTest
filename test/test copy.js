...
let testSuite = global.testSuites;

for (let j=0; j < testSuite.length; j++){
    // console.log(testSuite[j])
    if(testSuite[j].skip !='y' && testSuite[j].skip !='Y'){
        let testSet = testSuite[j].scenario;
        if(hasData(testSuite[j].testPlan)){
            testPlanId = testSuite[j].testPlan;
        }
        if(hasData(testSuite[j].testExec)){
            testExecutionId = testSuite[j].testExec;
        }
